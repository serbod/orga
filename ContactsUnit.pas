...
begin
  if Index < self.Count then
    result:=(self.Items[Index] as TDbItem)
  else
    result:=nil;
end;
...
function TContDataList.NewItem(): TDbItem;
var
...
function TContactsList.NewItem(): TDbItem;
var
...
begin
  ...
  begin
    ...
    if Assigned(TmpItem) then TmpItem.DataList.Add(TmpDI)
    else AllContData.Delete(n);
  end;
...
begin
  ...
  if result=0 then result := AnsiCompareStr(TContactItem(Item1).Desc, TContactItem(Item2).Desc);
end;
...
begin
  ...
  for i:=0 to self.Count-1 do
  begin
    if (self.GetItem(i) as TContactItem).ParentID = id then Inc(result);
  end;
end;
